...
export default function Cookies() {
  ...
  const createCookie = () => {
    ...
    if (cookieName !== '' && cookieValue !== '') {
        handleErrorWindow(cookiesArray.includes(cookieName) ? 'Vous avez modifié votre cookie' : 'Vous avez créé un nouveau cookie');
  ...
  const CookieCard = props => {
    ...
    return (
      <div className="cookie-card">
        <p><span>Name : </span>{cookie.name}</p>
        <p><span>Valeur : </span>{cookie.value}</p>
        <button onClick={e => deleteCard(props.index)}><p>Supprimer</p></button>
      </div>
    )
  }
  ...
  return (
    <div className="cookies-page">
      ...
      {showErrorWindow && <div className="alert-container">
        <div className="alert-window">
          <p>{errorMessage}</p>
        </div>
      </div>}
      <h1><span>Création de</span> Cookies 🍪</h1>
      <form>
        <div className="input-container">
          <label htmlFor="cookie-name">Son nom</label>
          <input type="text" name="cookie-name" id="cookie-name" onChange={handleCookie} value={cookieObject.name} required />
          <button className="reset" onClick={e => resetObjectKey(0)}>X</button>
        </div>
        <div className="input-container">
          <label htmlFor="cookie-value">Sa valeur</label>
          <input type="text" name="cookie-value" id="cookie-value" onChange={handleCookie} value={cookieObject.value} required />
          <button className="reset" onClick={e => resetObjectKey(1)}>X</button>
        </div>
      </form>
      <div className="buttons-container">
        <button className="create-button" onClick={createCookie} >Créer</button>
        <button className="display-button" onClick={showCookies}>{cookiesVisibles ? 'Masquer' : 'Afficher'}</button>
      </div>
      {cookiesVisibles && <div className="cookie-container">
        {cookiesArray.length ? cookiesArray.map((cookie,index) => <CookieCard cookie={cookie} index={index} key={uuidv4()} />) : <p>Pas de cookies pour le moment</p>}
      </div>}
      </main>
    </div>
  )
